    ...
    public partial class PriceUsd
    {
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.Categorical.OneHotEncoding(new []{new InputOutputColumnPair(@"storage_type", @"storage_type"),new InputOutputColumnPair(@"operating_system", @"operating_system"),new InputOutputColumnPair(@"target_use_case", @"target_use_case")}, outputKind: OneHotEncodingEstimator.OutputKind.Indicator)      
                                    .Append(mlContext.Transforms.ReplaceMissingValues(new []{new InputOutputColumnPair(@"cpu_speed_ghz", @"cpu_speed_ghz"),new InputOutputColumnPair(@"cpu_cores", @"cpu_cores"),new InputOutputColumnPair(@"ram_gb", @"ram_gb"),new InputOutputColumnPair(@"storage_gb", @"storage_gb")}))      
                                    .Append(mlContext.Transforms.Concatenate(@"Features", new []{@"storage_type",@"operating_system",@"target_use_case",@"cpu_speed_ghz",@"cpu_cores",@"ram_gb",@"storage_gb"}))      
                                    .Append(mlContext.Regression.Trainers.FastForest(new FastForestRegressionTrainer.Options(){NumberOfTrees=4,NumberOfLeaves=4,FeatureFraction=1F,LabelColumnName=@"price_usd",FeatureColumnName=@"Features"}));
