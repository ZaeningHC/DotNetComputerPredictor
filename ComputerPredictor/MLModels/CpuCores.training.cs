    ...
    public partial class CpuCores
    {
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.Categorical.OneHotEncoding(new []{new InputOutputColumnPair(@"storage_type", @"storage_type"),new InputOutputColumnPair(@"operating_system", @"operating_system"),new InputOutputColumnPair(@"target_use_case", @"target_use_case")}, outputKind: OneHotEncodingEstimator.OutputKind.Indicator)      
                                    .Append(mlContext.Transforms.ReplaceMissingValues(new []{new InputOutputColumnPair(@"cpu_speed_ghz", @"cpu_speed_ghz"),new InputOutputColumnPair(@"ram_gb", @"ram_gb"),new InputOutputColumnPair(@"storage_gb", @"storage_gb"),new InputOutputColumnPair(@"price_usd", @"price_usd")}))      
                                    .Append(mlContext.Transforms.Concatenate(@"Features", new []{@"storage_type",@"operating_system",@"target_use_case",@"cpu_speed_ghz",@"ram_gb",@"storage_gb",@"price_usd"}))      
                                    .Append(mlContext.Transforms.Conversion.MapValueToKey(outputColumnName:@"cpu_cores",inputColumnName:@"cpu_cores",addKeyValueAnnotationsAsText:false))      
                                    .Append(mlContext.MulticlassClassification.Trainers.OneVersusAll(binaryEstimator:mlContext.BinaryClassification.Trainers.FastTree(new FastTreeBinaryTrainer.Options(){NumberOfLeaves=4,MinimumExampleCountPerLeaf=20,NumberOfTrees=4,MaximumBinCountPerFeature=254,FeatureFraction=1,LearningRate=0.09999999999999998,LabelColumnName=@"cpu_cores",FeatureColumnName=@"Features",DiskTranspose=false}),labelColumnName: @"cpu_cores"))      
